(function($){
	...
	test("pull in optgroup's class", function(){
		...
		elems.filter(":not(:last)").each( function() {
			equals($(this).hasClass('wselect-optgroup-label'),true,'Default class is present when no extra class is defined');
		});
		elems.filter(":last").each( function() {
			equals($(this).hasClass('wselect-optgroup-label'),true,'Default class is present when extra class is defined');
