...
module.exports = function(grunt) {
  ...
  grunt.registerMultiTask('qunit-cov', 'Run QUnit unit tests in a headless PhantomJS instance.', function() {
    // Get files as URLs.
    var urls = grunt.file.expandFileURLs(this.data.files);
    ...
    grunt.utils.async.forEachSeries(urls, function(url, next) {
      ...
      var tempfile = { path: 'p:\\temp.temp', unlink: function(){} };
  ...
  grunt.registerHelper('phantomjs', function(options) {
    ...
    function PrintCoverage(coverageInfo, minimum, srcDir, outDir)
    {
        ...
        for(var key in coverageInfo)
        {
            ...
            for (var idx=0; idx < lineCoverage.length; idx++)
            {
                ...
                var coverageBase = fs.readFileSync('coverageBase.htm', 'utf8');
                //console.log(fileLines.length + ' x ' + lineCoverage.length);
    ...
    {
        filename = filename;
        return grunt.file.read(filename).split(/\r?\n/);
    }
};